...
def exp(rv):
    ...
    if (rv[1] != -0.):
      ...
      result = np.array([[np.cos(theta) + vector[0]**2*(1-np.cos(theta)), vector[0] * vector[1]*(1-np.cos(theta)) - vector[2] * np.sin(theta), vector[0] * vector[2]*(1-np.cos(theta))+vector[1]*np.sin(theta)],
                        [vector[1] * vector[0] * (1-np.cos(theta)) + vector[2] * np.sin(theta), np.cos(theta) + vector[1]**2*(
                            1-np.cos(theta)), vector[1]*vector[2]*(1-np.cos(theta)) - vector[0] * np.sin(theta)],
...
def log(R):
    if(R[0, 0]+R[1, 1]+R[2, 2] == 3):
        ...
        vector = np.array([(R[2, 1]-R[1, 2])/(2*np.sin(theta)), (R[0, 2]-R[2, 0]) /
                          (2*np.sin(theta)), (R[1, 0]-R[0, 1])/(2*np.sin(theta))]) * theta
...
def drawCubes(brightness):
    ...
    glMaterialfv(GL_FRONT, GL_AMBIENT_AND_DIFFUSE,
                 (.5*brightness, .5*brightness, .5*brightness, 1.))
...
def render(ang):
    ...
    euler2 = np.array([0., 0., 1.])*np.radians(90)
